classify.200.rf <- function(per) {
	...
	all.train <- two.class[ind==1, 1:21]
	all.test <- two.class[ind==2, 1:21]
...
#	accuracySum <- c("AccuracyAll:", accuracy)
	...
	accFile <- paste("./performance_summary/accuracy_RF_http_smtp_noPCA_per", per,"_200.csv", sep="")

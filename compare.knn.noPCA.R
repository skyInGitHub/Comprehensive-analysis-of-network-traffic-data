# Compare two classes
compare.knn.noPCA <- function (classTable, vk, per) {
	library(class)
	...
	for (i in 1:endL)
	{
		...
		for (j in (i+1):classInd)
		{
			...
			newFileAdd <- paste("./performance_summary/", classList[i], "_", classList[j], "_noPCA_knn", vk, "_", per, ".csv", sep="")
	...
	sumFile <- paste("./performance_summary/", "sum_noPCA_knn", vk, "_", per, ".csv", sep="")

# two classes with knn(k=1) and training set is precentage (no set.seed)
# in here k=1, PCs' number and percentage of entire set can be changed

# Set workplace
#setwd("/R_Data/Honours3")

# Before start this program, those libraries must be installed and used
#library(class)
#library(gmodels)

# Read all class table
#readTable <- function(){
#	classTable <- list()
#	classInd <- 0
#	for (cn in classList) {
#		fileCN <- paste("./class_table/", cn, "_table.csv", sep="")
#		ct.raw <- read.csv(fileCN)
#		ct <- ct.raw[, 2:23]
#		classInd <- classInd + 1
#		classTable[[classInd]] <- ct
#	}
#	names(classTable) <- classList
#}


# Compare two classes
compare.knn.PCA <- function (classTable, np, vk, per) {
	library(class)
	...
	for (i in 1:endL)
	{
		...
		for (j in (i+1):classInd)
		{
			...
			#c1c2.p.knn.table <- CrossTable(x=c1c2.testLabel, y=c1c2.p.knn, prop.chisq=FALSE)
			#accuracy <- (c1c2.p.knn.table$t[1,1] + c1c2.p.knn.table$t[2,2])/nrow(c1c2.test)
...
#			write.csv(SumT$importance, file=newSummaryAdd)
			...
			newFileAdd <- paste("./performance_summary/", classList[i], "_", classList[j], "_p", np, "_knn", vk, "_", per, ".csv", sep="")
			#Acc <- paste("Accuracy: ", accuracy, sep="")
			accTable <- rbind(c1c2.p.knn.table, c("Accuracy:", accuracy))
			write.csv(accTable, file=newFileAdd)
		
			#write.table(Acc, file=newFileAdd)
			#sink(file=newFileAdd, append=TRUE)
			#CrossTable(x=c1c2.testLabel, y=c1c2.p.knn, prop.chisq=FALSE)
			#sink()
	...
	sumFile <- paste("./performance_summary/", "sum_p", np, "_knn", vk, "_", per, ".csv", sep="")
